/**
 * Three.js coffee bean that rotates, breathes smoke, and cracks open on
 * click/drag to reveal the BeviGrow logo.
 *
 * The bean is built from two half-spheres sharing a seam, so "cracking" is
 * just animating the halves apart along X — which reads exactly like a real
 * bean splitting along its crease.
 */
...
/** Pure-CSS bean shown when WebGL is unavailable, so the intro never breaks. */
        ...
        {[1, -1].map((side) => (
          <div
            ...
            style={{
              ...
              borderRadius: side === 1 ? '0 100% 100% 0 / 0 50% 50% 0' : '100% 0 0 100% / 50% 0 0 50%',
...
export function BeanScene({ phase, onCrack }: { phase: Phase; onCrack: () => void }) {
  ...
  if (!webgl) return <CssBean phase={phase} onCrack={onCrack} />
