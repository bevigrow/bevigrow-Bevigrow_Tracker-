...
class Product(Base):
    ...
    category_id: Mapped[int] = mapped_column(ForeignKey("bs_categories.id"), index=True)
...
class Location(Base):
    ...
    stock_movements: Mapped[list["StockMovement"]] = relationship(back_populates="location", cascade="all, delete-orphan")
...
class Inventory(Base):
    ...
    __table_args__ = (
        __import__('sqlalchemy').UniqueConstraint('product_id', 'location_id', name='uq_product_location'),
    )
